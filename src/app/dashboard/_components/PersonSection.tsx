...
export default function PersonSection({ name, timeEntries, tasks, projects }: Props) {
        ...
        <div className="flex items-center gap-3 text-xs text-gray-500">
          ...
          {tasks.length > 0 && (
            <span>
              ...
              {billableCount > 0 && <span className="text-emerald-600 font-medium"> · {billableCount} billable</span>}
            </span>
          )}
          {projects.length > 0 && (
      ...
      <div className="divide-y divide-gray-100 px-4">
        ...
        {timeEntries.length > 0 && (
          <div className="py-4 space-y-2">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Time Entries</h3>
              ...
              {tasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between px-4 py-2.5 text-sm">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {task.status && (
                      <span className={`shrink-0 inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[task.status] ?? 'bg-gray-100 text-gray-600'}`}>
                        {task.status}
                      </span>
                    )}
                  </div>
        ...
        {projects.length > 0 && (
          <div className="py-4 space-y-2">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Projects</h3>
            <div className="rounded-lg border border-gray-200 divide-y divide-gray-100">
        ...
        {timeEntries.length === 0 && tasks.length === 0 && projects.length === 0 && (
          <p className="py-4 text-sm text-gray-400">No activity in this period.</p>
        )}
      </div>
    </div>
  )
